...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        if state not in self.consideredStates:
            return 0
        else:
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        if state is not None:
            nextUtility = self.computeValueFromQValues(nextState)
            self.values[(state, action)] = self.values[(state, action)] + self.alpha * (reward + self.discount*nextUtility - self.values[(state, action)])
    ...
    def getQValue(self, state, action):
        ...
        if state == 'TERMINAL_STATE':
            ...
            for feature in self.featExtractor.getFeatures(state,action):
                featureVal = self.featExtractor.getFeatures(state,action)[feature]
