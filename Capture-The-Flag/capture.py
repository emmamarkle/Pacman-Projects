...
def save_weights_off(agent):
  with open('OffensiveWeights','w') as f:
    for weight in agent.weights:
      print(agent.weights[weight], file=f)

def save_weights_def(agent):
  with open('DefensiveWeights','w') as f:
    for weight in agent.weights:
      print(agent.weights[weight], file=f)      
...
def main():
    gameState = GameState()
    agent = training.DefensiveReflexAgent(2)
    numExperiences = 2
    oldweights = util.Counter()


    for i in range(numExperiences):
        agent.weights = oldweights
        options = readCommand(game.sys.argv[1:])  # Get game components based on input
        games = runGames(**options)
        save_weights_def(agent)
        oldweights = agent.getWeights(gameState, "North")
        save_score(games[0])
    return oldweights
